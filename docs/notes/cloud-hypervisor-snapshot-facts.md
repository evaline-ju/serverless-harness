...
**Hardware-test pass (2026-09-11):** `docs/notes/cloud-hypervisor-tutorial.md`
put a real Cloud Hypervisor v53.0 install through the same snapshot/restore,
...
- **Row 2** (multi-resume insecurity): documented as an intended feature with
  ...
  doc-asserted** (`cloud-hypervisor-tutorial.md` §6b-6c) — the same snapshot
...
- **New finding, no row of its own — advisory write-lock on disk images**
  (`cloud-hypervisor-tutorial.md` §9b): restoring a _second_ process against a
...
- **New finding, no row of its own — page-cache sharing across restored VMs is
  the open item, not a green light** (`cloud-hypervisor-tutorial.md` §9, in
  full): the documented mechanism for it (`memory_restore_mode=CopyOnWrite`)
...
- **Row 6** (cgroups v1 restore latency): still undocumented, and the
  ...
  not measured. Related but distinct: §10's informal timing table found
  Cloud Hypervisor's default-mode restore (~141 ms) markedly slower than
  Firecracker's (~37 ms) on the same box, though that gap is attributed to
  `Copy` mode's eager memory-ranges copy, not to cgroup version — a different
  variable than row 6 asks about, so row 6 itself stays undocumented rather
  than hardware-confirmed either way.
...
- **Row 11**: virtio-fs is present in Cloud Hypervisor, and now
  **hardware-confirmed working** (`cloud-hypervisor-tutorial.md` §8) — two
